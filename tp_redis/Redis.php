...
//          echo "$key - $value \n";
    ...
    public function sIsMember($key,$member){
        ...
        if(!$re) return false;
        return $this->redisObj[$this->sn]->sIsMember($key,$member);
    }
    public function sRemoveMember($key,$member){
        return $this->redisObj[$this->sn]->sRem($key,$member);
    }
     ...
     * @return array
     */
    public function zRangByScore($key,$star,$end,$withScore=false){
        if ( $withScore ){
            return $this->redisObj[$this->sn]->zRevRange($key,$star,$end,true);
        }
    ...
    public function  zDelete ($key,$field){
        $num = 0;
        if (is_array( $field )){
            foreach ($field as $value ){
                $num +=  $this->redisObj[$this->sn]->zDelete($key,$value);
            }
        }else{
            $num +=$this->redisObj[$this->sn]->zDelete($key,$field);//返回删除个数
        }
        return $num;
    }

    public function zExists($key,$field){
        return ($this->zRank($key,$field) == false) ? false : true;
    }
    ...
    public function hExists($key,$field=''){
        $re = $this->redisObj[$this->sn]->hexists($key,$field);//有返回1，无返回0
        return $re;
    }
